...
bool FileReader::ReadFile(std::string *str) {
  ...
  std::size_t filename_len = fullfile.copy(filename, fullfile.length(), 0);
  filename[filename_len] = '\0';
  char *result = ::ReadFile(filename, &flen);
  if (result == NULL ) {
    return false;
  }
  std::unique_ptr<char, MallocDeleter<char>> fileptr(result);

  std::string tmp(result, flen);
  *str = tmp;

...
// This is the private helper value free function for the hashtable
static void ValueFreeFn(HTValue_t value);
...
void ValueFreeFn(HTValue_t value) {
  free(value);
}
...
void FreeDocTable(DocTable table) {
  ...
  // STEP 1.
  void (* ValueFreeFnPtr)(HTValue_t) = ValueFreeFn;
  FreeHashTable(table->docid_to_docname, ValueFreeFnPtr);
  FreeHashTable(table->docname_to_docid, ValueFreeFnPtr);
  free(table);
}
...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  if (res) {
    // The docname is already in the doctable, return the docid
    free(docid);
    free(doccopy);
    return res;
  }
  ...
  *docid = table->max_id;
  ...
  kv.key = *docid;
  kv.value = doccopy;
  int result = InsertHashTable(table->docid_to_docname, kv, &oldkv);
  Verify333(result != 0);
  ...
  kv.value = docid;
  result = InsertHashTable(table->docname_to_docid, kv, &oldkv);
  Verify333(result != 0);
  return *docid;
}

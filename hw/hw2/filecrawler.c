...
static void HandleDir(char *dirpath, DIR *d, DocTable *doctable,
  ...
  while (1) {
    ...
    // STEP 1.
    // Use the "readdir" system call to read the next directory entry. (man
    // readdir).  If we hit the end of the directory, return back
    ...
    if (res == 0) {
      ...
      if (S_ISDIR(nextstat.st_mode)) {
        DIR * tempd = opendir(newfile);
        if (tempd == NULL) {
          continue;
        }
        HandleDir(newfile, tempd, doctable, index);
        Verify333(closedir(tempd) == 0);
      }
    }

...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  uint32_t res = WriteHeader(f, dtres, mires);
  if (res == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  res = fwrite(&header, sizeof(docid_element_header), 1, f);
  if (res != 1 && ferror(f)) {
    return 0;
  }
  total += sizeof(docid_element_header);
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    res = fwrite(&position, sizeof(docid_element_position), 1, f);
    if (res != 1 && ferror(f)) {
      LLIteratorFree(it);
      return 0;
    }
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  total += sizeof(worddocset_header);
...
static HWSize_t WriteHeader(FILE *f,
  ...
  // Set the cursor to the start of the doctable
  ...
  for (uint32_t i = 0; i < cslen; i++) {
    ...
    crcobj.FoldByteIntoCRC((uint8_t)nextbyte);
  }
  ...
  if (fseek(f, 0, SEEK_SET) != 0)
    return 0;
  if (fwrite(&header, sizeof(IndexFileHeader), 1, f) != 1 && ferror(f))
    return 0;
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  bucket_rec br;
  // MISSING:
  uint32_t chain_len = NumElementsInLinkedList(li);
  br = {chain_len, b_offset};
  br.toDiskFormat();
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      element_position_rec elePosRec = {nextelpos};
      elePosRec.toDiskFormat();
      ...
      // Writes the element position in doctable
      if (fwrite(&elePosRec, sizeof(element_position_rec), 1, f) != 1
            && ferror(f)) {
        LLIteratorFree(it);
        return 0;
      }
      offset += sizeof(elePosRec);
      // Call write element function
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    HWSize_t br_retval = 0;

    if (NumElementsInLinkedList(li) != 0) {
      WriteBucketRecord(f, li, next_bucket_rec_offset,
                        next_bucket_offset);
      br_retval = WriteBucket(f, li, next_bucket_offset, fn);
    } else {
      WriteBucketRecord(f, li, next_bucket_rec_offset,
          next_bucket_offset);
    }
    next_bucket_offset += br_retval;

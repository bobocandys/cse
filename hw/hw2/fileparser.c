...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  if (result == -1) {
    // Error encounted when opening the file
    return NULL;
  }
  fd = result;
  // STEP 4.
  ...
  // Use cur to keep track of where we should start loading data
  ...
  while (left_to_read > 0) {
    ...
    if (numread == -1) {
      ...
      if (errno == EAGAIN || errno == EINTR) {
        // Recoverable error, just try again
        continue;
      } else {
        // Uncoverable error
        free(buf);
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  // it udnerlined with "=", and boundary characters underlined
  // with "+":
  //
  ...
  while (1) {
    ...
    // Move cur to the next character
    curptr++;
  }
}
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    strncpy(newstr, word, 1 + strlen(word));
    wp->word = newstr;
    ...
    kv.key = hashKey;
    kv.value = wp;
    retval = InsertHashTable(tab, kv, &temp);
    Verify333(retval != 0);
  }
}

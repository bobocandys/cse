...
bool PopLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  *payload_ptr = temp->payload;
  ...
  if (list->num_elements == 1) {  // single element list
    list->num_elements = 0;
    list->head = NULL;
    list->tail = NULL;
  } else {
...
bool AppendLinkedList(LinkedList list, LLPayload_t payload) {
  ...
  // When there is >= 1 nodes in the list
  ln->next = NULL;
  ln->prev = list->tail;
  (list->tail)->next = ln;
  list->tail = ln;
...
bool LLIteratorDelete(LLIter iter,
  ...
  if (iter->list->num_elements == 1) {
    // Case when the list becomes empty after deleting
    free(iter->node);  // Free the node
    ...
    free(iter->node);
    iter->node = iter->list->head;  // reassigns the reference
    iter->node->prev = NULL;

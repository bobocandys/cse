...
HashTableReader::HashTableReader(FILE *f, IndexFileOffset_t offset)
  ...
  // MISSING:
  Verify333(fseek(f, offset, SEEK_SET) != -1);
  int res = fread(&header_, 4, 1, file_);
  Verify333(res != 0 && !ferror(f));
...
HashTableReader::LookupElementPositions(HTKey_t hashKey) {
  ...
  bucket_rec b_rec;
  // MISSING:
  Verify333(fseek(file_, bucketrec_offset, SEEK_SET) != -1);
  int res = fread(&b_rec, sizeof(bucket_rec), 1, file_);
  Verify333(res == 1);
  b_rec.toHostFormat();
  ...
  // MISSING:
  Verify333(fseek(file_, b_rec.bucket_position, SEEK_SET) != -1);
  for (uint32_t i = 0; i < b_rec.chain_len; i++) {
    element_position_rec pos;
    int res = fread(&pos, 4, 1, file_);
    Verify333(res == 1);
    pos.toHostFormat();

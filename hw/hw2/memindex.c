...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    // Allocate a WordDocSet
    wds = (WordDocSet *)malloc(sizeof(WordDocSet));
    if (wds == NULL) {
      return 0;
    }

    // Preparing allocated WordDocSet
    wds->word = (char *)malloc(1 + strlen(word));
    if (wds->word == NULL) {
      free(wds);
      return 0;
    }
    strncpy(wds->word, word, 1 + strlen(word));
    free(word);

    wds->docIDs = AllocateHashTable(3);
    if (wds->docIDs == NULL) {
      free(wds);
      free(wds->word);
      return 0;
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  Verify333(res != -1);
  if (res == 0) {
    // If we did not find any matching with query[0]
    FreeLinkedList(retlist, (LLPayloadFreeFnPtr)free);
    return NULL;
  }

  // The corresponding matching WordDocSet
  wds = kv.value;
  docIDTable = wds->docIDs;

  // Make a iterator on the docID table
  it = HashTableMakeIterator(docIDTable);
  Verify333(it != NULL);

  do {
    // Loop through each matching document
    res = HTIteratorGet(it, &kv);
    if (!res) {
      // If we fail to get keyValue from iterator
      HTIteratorFree(it);
      FreeLinkedList(retlist, (LLPayloadFreeFnPtr)free);
      return NULL;
    }
    ...
    result->docid = kv.key;
    result->rank = NumElementsInLinkedList(kv.value);
    Verify333(AppendLinkedList(retlist, result));
  } while (HTIteratorNext(it));
  ...
  for (i = 1; i < qlen; i++) {
    ...
    // The WordDocSet for this query[i]
    wds = kv.value;
    docIDTable = wds->docIDs;
    ...
    for (j = 0; j < ne; j++) {
      // Get the docID data from the searchResult node
      SearchResult *data;
      LLIteratorGetPayload(llit, (void **)&data);

      res = LookupHashTable(docIDTable, data->docid, &kv);
      Verify333(res != -1);
      if (res == 0) {
        ...
        // Update the rank
        data->rank += NumElementsInLinkedList(kv.value);
        if (!LLIteratorNext(llit)) {
          break;
        }
      }
    }
